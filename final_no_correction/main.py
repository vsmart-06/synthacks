...
class login:
    ...
    def login(self):
        ...
        if self.db.is_user(username, password):
            ...
            self.window.destroy()
            self.window = tk.Tk()
            todo_button = tk.Button(self.window, text = "To Do List", command = lambda m = self.window: m.destroy())
            summary_button = tk.Button(self.window, text = "Text Summarizer")
            spam_button = tk.Button(self.window, text = "Spam Classifier")
            todo_button.pack()
            summary_button.pack()
            spam_button.pack()
            self.window.mainloop()
